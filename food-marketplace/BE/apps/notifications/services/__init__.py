...
def notify_combo_available(db: Session, combo_id: str, shop_name: str, locker_name: str, locker_address: str = ""):
    ...
    price_str = f"{combo.sale_price:,}₽".replace(",", ".")
    discount_str = f"-{combo.discount_rate}%"
    location_line = f"{locker_name}" + (f" · {locker_address}" if locker_address else "")

    title = f"Новый набор от {shop_name}!"
    body = (
        f"{combo.title}\n"
