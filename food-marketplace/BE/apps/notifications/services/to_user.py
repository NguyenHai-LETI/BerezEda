...
def notify_order_expired(db: Session, order_id: str, customer_id: str):
    """Notify customer that their pickup window has closed and locker has been released."""
    from apps.orders.models import Order
    ...
    combo = db.exec(select(Combo).where(Combo.id == order.combo_id)).first()
    user = db.exec(select(User).where(User.id == customer_id)).first()
    if not combo or not user:
        return

    locker_name = ""
    unit = db.exec(select(LockerUnit).where(LockerUnit.id == order.locker_unit_id)).first()
    if unit:
        from apps.lockers.models import LockerLocation
        loc = db.exec(select(LockerLocation).where(LockerLocation.id == unit.location_id)).first()
        if loc:
            locker_name = loc.name or ""

    template = NOTIFICATION_TEMPLATES[NotificationType.COMBO_PICKUP_DEADLINE_PASSED]
    send_push_notification(
        db,
        user,
        template,
        combo,
        NotificationType.COMBO_PICKUP_DEADLINE_PASSED,
        extra_data={"locker_name": locker_name},
    )
...
def handle_new_combo_published(combo_id: str, shop_id: str):
    ...
    with Session(engine) as session:
        ...
        # Query favorite users directly (Favorite model has no ORM relationship to User)
        favorites = session.exec(
            select(Favorite).where(Favorite.shop_id == shop_id)
        ).all()
        if not favorites:
            return
        ...
        template = NOTIFICATION_TEMPLATES[NotificationType.NEW_COMBO_FROM_FAVORITE_STORE]
