    ...
    with Session(engine) as session:
        ...
        send_push_notification(
            ...
            extra_data={
                ...
                or "レビューコメントがありません",  # Review comment or default text
            },
        )
...
def cleanup_expired_combos_and_lockers_service(
    ...
    This scheduled task:
    - Marks expired combos as EXPIRED
    ...
    expired_combos_stmt = select(Combo).where(
        ...
        col(Combo.status) == COMBO_STATUS_SELLING,
    )
    ...
    for combo in expired_combos:
        try:
            ...
            complete_reservation_and_release_locker(db, combo.id)
