...
class User(SQLModel, table=True):
    ...
    shop_id: Optional[str] = Field(
        default=None, foreign_key="shop.id", index=True
    )  # For staff members
    ...
    devices: List["Device"] = Relationship(back_populates="user")  # type: ignore
    myna_authorizers: List["MynaAuthorizer"] = Relationship(back_populates="user")  # type: ignore
    welfare_supports: List["UserWelfareSupport"] = Relationship(back_populates="user")  # type: ignore
    reviews: List["Review"] = Relationship(back_populates="user")  # type: ignore
    ...
    def get_associated_shop(self):
        """Get the shop associated with this user (owned shop for owners, working shop for staff)"""
        # Check permissions first to differentiate between owners and staff
        if self.permissions == "staff_shop" and self.shop:
            return self.shop
        elif self.permissions == "owner_shop" and self.owned_shop:
            return self.owned_shop
        # Fallback for backward compatibility
        elif self.role == "owner_shop" and self.owned_shop:
            return self.owned_shop
        return None
